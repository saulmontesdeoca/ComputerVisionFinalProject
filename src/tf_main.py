...
parser.add_argument('-i', '--input_video', type=str,
                    default='input/video_1.mp4', help='input video file')
...
with open('model_files/object_detection_classes_coco.txt', 'r') as f:
...
# load the DNN model
mobile_net_model = cv2.dnn.readNet(model='model_files/frozen_inference_graph.pb',
                                   config='model_files/ssd_mobilenet_v2_coco_2018_03_29.pbtxt.txt',
...
def drawDetections(output, img, copy_image):
    ...
    for detection in output[0, 0, :, :]:
        ...
        if confidence > .3:
            ...
            cv2.rectangle(copy_image, (int(box_x), int(box_y)),
                          (int(box_width), int(box_height)), color, thickness=2)

            cv2.putText(copy_image, class_name, (int(box_x), int(
                box_y - 5)), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
            cv2.putText(copy_image, str(round(confidence, 2)), (int(
                box_x + 120), int(box_y - 5)), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
    return copy_image
...
def getBlob(img):
    ...
    blob = cv2.dnn.blobFromImage(image=img, size=(700, 700), mean=(
